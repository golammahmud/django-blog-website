...
def PostListView(request):
    ...
    if category_id:
        posts=Post.get_all_post_by_category(category_id)
    else:
...
def search(request):
    ...
    #     messages.warning(request,'no search results found..please try again proper query')
        ...
        post = Post.objects.filter(Q(category__in=category) |Q(author__in=authors) | Q(title__icontains=query) | Q(content__icontains=query)
                                   | Q(pk__icontains=[query]) | Q(date_created__date__icontains=query))
    # date_created
    context = {
        'posts': post
    }
